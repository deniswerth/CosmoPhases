...
class bubble_wall_velocity():
    ...
    def plot_velocity_sensitivity(self):
        ...
        ax1.contourf(Tn1, phi01, vw[0], 20, vmin = np.min(v), vmax = np.max(v))
        ...
        ax2.contourf(Lw2, phi02, vw[1], 20, vmin = np.min(v), vmax = np.max(v))
        ...
        ax3.contourf(Tn3, Lw3, vw[2], 20, vmin = np.min(v), vmax = np.max(v))
