...
class Finite_Temperature_Potential(generic_potential.generic_potential):
    ...
    def init(self, mDelta, LambdaDelta, kappa):
        ...
        #Renormalization scales for goldstone bosons
        #self.renormScaleSq_chi = self.mH0**2
        #self.renormScaleSq_xi = mH0**2
    ...
    def boson_massSq(self, X, T):
        ...
        #Thermal corrections
        Pi_H = T**2*( 3*g**2/16 + gprime**2/16 + yt**2/4 + LambdaH/2 + kappa/24 ) #SM-like Higgs boson 
        Pi_Delta = T**2*( kappa/6 + LambdaDelta/2 ) #Higgs triplet
        ...
        mZ_Sq_field = A + C + np.sqrt(np.abs( (A - C)**2 + 4*B**2 ))/2

        #Squared masses at the VEV for (h, sigma) = (v0, 0)
        mH_Sq_vev = -muH2 + 3*LambdaH*v0**2 + 0*h
        mDelta_Sq_vev = -muDelta2 + kappa*v0**2/2 + 0*h
        mchi_Sq_vev = -muH2 + 3*LambdaH*v0**2 + 0*h
        mxi_Sq_vev = -muDelta2 + kappa*v0**2/2 + 0*h
