...
class Phase_Transition():
    ...
    def today_universe(self):
        ...
        FTP = self.FTP
        T = 0
        if self.v0**2*self.mH0**2/2 > ( self.kappa*self.v0**2/2 - self.mDelta**2 )**2/self.LambdaDelta:
        #if FTP.DVtot(FTP.findMinimum(X = [1000, 0], T = T), T = T) < FTP.DVtot(FTP.findMinimum(X = [0, 1000], T = T), T = T) and abs(FTP.findMinimum(X = [1000, 0], T = 0)[0] - FTP.v0) < 10:
            return 1
        else:
            return 0
